...
def visualize_policy(i):
    ...
    pidf.columns = ['Ace', 'Dealer', 'Player', 'Action']
    for ace in (0, 1):
        acedf = pidf.loc[pidf['Ace']==ace][['Dealer', 'Player', 'Action']]
        ...
        fig.write_image('blackjack_es_pi_iteration_' + str(i) + '_ace' + str(ace) + '.png')
    ...
    for ace in (0, 1):
        ...
        fig.update_xaxes(side="top")
        fig.write_image('blackjack_es_v_iteration_' + str(i) + '_ace' + str(ace) + '.png')
...
for e in range(0, 1000000+1):
    ...
    for t in reversed(range(0, T-1)):
        ...
        if (t == 0) or ((S[t], A[t]) not in zip(S[0:t-1], A[0:t-1])):
            ...
            pi[S[t]] = 1 if Q[(S[t], 1)] > Q[(S[t], 0)] else 0
        else:
            pass
